...
class BlogPostDAO:
    ...
    def add_comment(self, permalink, name, email, body):
	    ...
	    # 1. find using permalink		
	    post = self.posts.find({'permalink': permalink})
	    print 'post=', post
	    # 2. update comments in 	    
	    comments = self.posts.find_one({'permalink': permalink}, {'_id': 0, 'comments': 1});
	    comments = comments['comments'];
	    #print 'comments=', comments, '>' , comment
	    comments.append(comment);
	    #print 'comments=', comments
	    r = self.posts.update_one({'permalink': permalink},{'$set': {'comments': comments}}); 
	
     	    return r.matched_count          # return the number of documents updated
